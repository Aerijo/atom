...
class CreateRpmPackage extends Task {
  constructor() {
    super("Create RPM package");
  }

  run(packagedAppPath) {
    this.subtask(`Creating rpm package for "${packagedAppPath}"`);
    ...
    this.subtask(`Copying icons into "${rpmPackageIconsDirPath}"`);
    fs.copySync(
      path.join(
    ...
    this.subtask(`Writing rpm package spec file into "${rpmPackageSpecsDirPath}"`);
    ...
    this.subtask(`Writing desktop entry file into "${rpmPackageBuildDirPath}"`);
...
module.exports = new CreateRpmPackage();
