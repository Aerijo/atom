...
class CreateDebianPackage extends Task {
  constructor() {
    super("Create Debian package");
  }

  run(packagedAppPath) {
    this.subtask(`Creating Debian package for "${packagedAppPath}"`);
    ...
    this.subtask(`Copying "${packagedAppPath}" to "${debianPackageAtomDirPath}"`);
    fs.copySync(packagedAppPath, debianPackageAtomDirPath);
    fs.chmodSync(debianPackageAtomDirPath, '755');

    this.subtask(`Copying binaries into "${debianPackageBinDirPath}"`);
    fs.copySync(
    ...
    this.subtask(`Writing control file into "${debianPackageConfigPath}"`);
    ...
    this.subtask(`Copying icon into "${debianPackageIconsDirPath}"`);
    fs.copySync(
      path.join(
        packagedAppPath,
        'resources',
    ...
    this.subtask(`Copying license into "${debianPackageDocsDirPath}"`);
    fs.copySync(
    ...
    this.subtask(`Generating .deb file from ${debianPackageDirPath}`);
...
module.exports = new CreateDebianPackage();
