...
class GenerateStartupSnapshot extends Task {
  constructor() {
    super("Generate startup snapshot");
  }

  run(packagedAppPath) {
    this.subtask(`Generating snapshot for ${packagedAppPath}`);
    ...
    return electronLink({
      ...
      this.info('Minifying startup script');
      ...
      this.info('Verifying if snapshot can be executed via `mksnapshot`');
      ...
      this.info('Generating startup blob with mksnapshot');
      ...
      for (let snapshotBinary of snapshotBinaries) {
        ...
        this.info(`Moving generated startup blob into "${destinationPath}"`);
        try {
...
module.exports = new GenerateStartupSnapshot();
